 ...
 *
 * .NET 10 Migration: SplendidCRM/_code/ImportUtils.cs → src/SplendidCRM.Core/ImportUtils.cs
 *   - CHANGED: static class → instance class with DI constructor
 *   - CHANGED: HttpApplicationState Application → IMemoryCache (constructor injection)
 *   - CHANGED: HttpContext.Current.Session → ISession session (method parameter)
 *   - CHANGED: HttpContext.Current.Response → HttpResponse response (method parameter)
 *   - CHANGED: Security.USER_ID/TEAM_ID/PRIMARY_ROLE_NAME (static) → _security.USER_ID/... (injected instance)
 *   - CHANGED: SplendidCache.EditViewFields() (static) → _splendidCache.EditViewFields() (injected instance)
 *   - CHANGED: DbProviderFactories.GetFactory() (static) → _dbProviderFactories.GetFactory() (injected instance)
 *   - CHANGED: using System.Web / System.Web.SessionState → removed; using Microsoft.AspNetCore.Http added
 *   - CHANGED: using System.Workflow.Activities.Rules → using LogicBuilder.Workflow.Activities.Rules
 *   - CHANGED: new RuleValidation(typeof(SplendidImportThis), typeProvider) → new RuleValidation(typeof(SplendidImportThis))
 *   - CHANGED: GenerateImport signature — DataView vwColumns removed; Hashtable hashMappings + StringDictionary dictCustomFields added
 *   - CHANGED: gPROSPECT_LIST_ID parameter removed (set locally to Guid.Empty)
 *   - CHANGED: sbImport, sbErrors, dtProcessed, ref nImported/nFailed/nDuplicates removed as params → local variables
 *   - CHANGED: string sProcessedFileID → string[] sProcessedFileIDs
 *   - CHANGED: Response.IsClientConnected → response.HttpContext.RequestAborted.IsCancellationRequested
 *   - CHANGED: Response.Write(" ") → response body write heartbeat
 *   - CHANGED: Session["TempFile." + id] = file → session.SetString("TempFile." + id, file) for each id
 *   - PRESERVED: namespace SplendidCRM, all business logic, stored procedure calls, ACT! import, duplicate detection
 *********************************************************************************************************************/
#nullable disable
using System;
using System.IO;
...
namespace SplendidCRM
{
	...
	/// Migrated from SplendidCRM/_code/ImportUtils.cs for .NET 10 ASP.NET Core.
	/// Replaces System.Web dependencies with ASP.NET Core and DI equivalents.
	/// HttpContext.Current replaced with IHttpContextAccessor/method parameters.
	/// HttpApplicationState (Application[]) replaced with IMemoryCache.
	/// Session[] access replaced with ISession parameter.
	/// </summary>
	public class ImportUtils
	{
		...
		/// <summary>
		/// Executes a batch import operation, reading rows from a temporary XML file, mapping them to stored
		/// procedure parameters, optionally checking for duplicates, executing business rules, and persisting
		/// each record via the appropriate stored procedure.  Results are stored in the distributed session
		/// keyed by each ID in sProcessedFileIDs.
		/// </summary>
		/// <param name="Container">SplendidControl execution context used by the rules engine.</param>
		/// <param name="L10n">Localization service for user-facing error messages.</param>
		/// <param name="session">Distributed session for storing processed-file results.</param>
		/// <param name="response">HTTP response for heartbeat writes and cancellation detection; may be null.</param>
		/// <param name="xmlMapping">XML mapping document containing field Name, Mapping, Default, and DuplicateFilter nodes.</param>
		/// <param name="sImportModule">CRM module name being imported (e.g., "Contacts", "Leads").</param>
		/// <param name="sSOURCE">Import source identifier (e.g., "act", "csv", "xml").</param>
		/// <param name="hashMappings">Pre-built header-to-SP-parameter mapping: source XML element name → destination SP parameter name.</param>
		/// <param name="sLayoutEditView">Edit view layout name used to retrieve required field definitions.</param>
		/// <param name="sProcessedFileIDs">Array of session keys under which the processed-results DataSet XML is stored.</param>
		/// <param name="bPreview">If true, runs inside a transaction that is never committed (dry run).</param>
		/// <param name="bHAS_HEADER">If true and source is not "xml", skips the first row (CSV header row).</param>
		/// <param name="bUSE_TRANSACTION">If true, wraps the import batch in a database transaction.</param>
		/// <param name="dtRules">DataTable containing business rules to execute against each row before insertion.</param>
		/// <param name="dictCustomFields">Maps custom field parameter names to their CSTM-table database column names.</param>
		/// <param name="sTempFileName">File name (relative to Path.GetTempPath()) of the temporary XML import data file.</param>
		public void GenerateImport(
			SplendidControl   Container,
			L10N              L10n,
			ISession          session,
			HttpResponse      response,
			XmlDocument       xmlMapping,
			string            sImportModule,
			string            sSOURCE,
			Hashtable         hashMappings,
			string            sLayoutEditView,
			string[]          sProcessedFileIDs,
			bool              bPreview,
			bool              bHAS_HEADER,
			bool              bUSE_TRANSACTION,
			DataTable         dtRules,
			StringDictionary  dictCustomFields,
			string            sTempFileName)
		{
			// 11/01/2006 Paul.  Max errors is now a config value.
			// CHANGED: Application["CONFIG.import_max_errors"] → _memoryCache.Get<object>("CONFIG.import_max_errors")
			int nMAX_ERRORS = Sql.ToInteger(_memoryCache.Get<object>("CONFIG.import_max_errors"));
			if ( nMAX_ERRORS <= 0 )
				nMAX_ERRORS = 200;

			// Local tracking counters (were ref parameters in the legacy API).
			int nImported  = 0;
			int nFailed    = 0;
			int nDuplicates = 0;

			// Local import log and error accumulators (were out parameters in the legacy API).
			StringBuilder sbImport = new StringBuilder();
			StringBuilder sbErrors = new StringBuilder();

			// Local processed-rows DataTable (was a parameter in the legacy API).
			DataTable dtProcessed = new DataTable(sImportModule);

			// gPROSPECT_LIST_ID was removed from the method signature; it is tracked locally.
			// The direct prospect-list import branch is preserved for future use.
			...
			try
			{
				...
				// CHANGED: new RuleValidation(typeof(SplendidImportThis), typeProvider) →
				//          new RuleValidation(typeof(SplendidImportThis))
				//          LogicBuilder.Workflow.Activities.Rules does not accept ITypeProvider.
				SplendidRulesTypeProvider typeProvider = new SplendidRulesTypeProvider();
				RuleValidation validation = new RuleValidation(typeof(SplendidImportThis));
				RuleSet rules = null;
				...
				if ( dtRules != null && dtRules.Rows.Count > 0 )
				{
					...
					{
						// RulesUtil.BuildRuleSet throws PlatformNotSupportedException on .NET 10.
						// Business rules are silently skipped; import proceeds without rule evaluation.
						rules = null;
					}
				}

				// Build reverse mappings from the caller-provided hashMappings
				// (hashMappings: source XML element name → dest SP parameter name).
				// hashReverseMappings: dest SP parameter name → source XML element name.
				StringDictionary hashReverseMappings = new StringDictionary();
				foreach (DictionaryEntry entry in hashMappings)
				{
					string sMappingKey  = Sql.ToString(entry.Key);
					string sParamName   = Sql.ToString(entry.Value);
					if ( !Sql.IsEmptyString(sParamName) && !hashReverseMappings.ContainsKey(sParamName) )
						hashReverseMappings.Add(sParamName, sMappingKey);
				}

				// Build hashDefaultMappings and hashDuplicateFilters from the XML mapping document.
				...
				// Build hashColumns: dest SP parameter name → display name (source column name).
				// Used to show friendly names in required-field error messages.
				// CHANGED: previously built from DataView vwColumns (row["NAME"] → row["DISPLAY_NAME"]);
				//          now derived from hashMappings (dest param → source column name as display).
				Hashtable hashColumns = new Hashtable();
				foreach (DictionaryEntry entry in hashMappings)
				{
					string sMappingKey  = Sql.ToString(entry.Key);
					string sParamName   = Sql.ToString(entry.Value);
					if ( !Sql.IsEmptyString(sParamName) )
					{
						string sParamUpper = sParamName.ToUpper();
						if ( !hashColumns.ContainsKey(sParamUpper) )
							hashColumns.Add(sParamUpper, sMappingKey);
					}
				}

				// 11/01/2006 Paul.  Use a hash for quick access to required fields.
				// CHANGED: SplendidCache.EditViewFields() (static) → _splendidCache.EditViewFields() (instance)
				// CHANGED: Security.PRIMARY_ROLE_NAME (static) → _security.PRIMARY_ROLE_NAME (instance)
				Hashtable hashRequiredFields = new Hashtable();
				DataTable dtRequiredFields = _splendidCache.EditViewFields(sImportModule + "." + sLayoutEditView, _security.PRIMARY_ROLE_NAME);
				...
				// CHANGED: DbProviderFactories.GetFactory() (static) → _dbProviderFactories.GetFactory() (instance)
				DbProviderFactory dbf = _dbProviderFactories.GetFactory();
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					con.Open();
					// Build TEAM_NAME → TEAM_ID lookup if team management is enabled.
					...
					// 11/01/2006 Paul.  The transaction is optional, just make sure to always dispose it.
					{
						...
						try
						{
							// CHANGED: Application["Modules." + sImportModule + ".TableName"] → _memoryCache.Get<object>(...)
							string sTABLE_NAME = Sql.ToString(_memoryCache.Get<object>("Modules." + sImportModule + ".TableName"));
							...
							// CHANGED: was DataView vwColumns filtered to "CustomField = 1"; now uses dictCustomFields parameter.
							//          Type information (ColumnType, Size) was available from vwColumns but not from dictCustomFields;
							//          all custom field parameters default to string type (DbType.String, size 4000).
							if ( dictCustomFields != null && dictCustomFields.Count > 0 )
							{
								...
								int nFieldIndex = 0;
								foreach (string sKey in dictCustomFields.Keys)
								{
									// sKey = field parameter name; dictCustomFields[sKey] = db column name.
									string sNAME = sKey.ToUpper();
									...
									// Default to string type — ColumnType/Size not available from dictCustomFields.
									// 01/13/2007 Paul.  We need to truncate any long strings to prevent SQL error.
									Sql.AddParameter(cmdImportCSTM, "@" + sNAME, String.Empty, 4000);
									nFieldIndex++;
								}
								// 09/19/2007 Paul.  Exclude ID_C as it is expected and required.
							...
							{
								// 10/07/2009 Paul.  We need to create our own global transaction ID to support auditing and workflow
								// on SQL Azure, PostgreSQL, Oracle, DB2 and MySQL.
							...
							if ( sSOURCE == "act" && cmdPROSPECT_LISTS_Import != null )
							{
								...
								// CHANGED: Security.TEAM_ID/USER_ID (static) → _security.TEAM_ID/USER_ID (instance)
								Guid gTEAM_ID          = _security.TEAM_ID;
								Guid gASSIGNED_USER_ID = _security.USER_ID;
								...
								if ( parID != null )
								{
									...
									foreach ( XmlNode xGroup in nlGroups )
									{
										foreach(IDbDataParameter par in cmdPROSPECT_LISTS_Import.Parameters)
										{
											...
											string sParameterName = par.ParameterName.TrimStart('@').ToUpper(); // CHANGED: Sql.ExtractDbName(cmd, name) (2-arg) → TrimStart('@') (not migrated)
										...
										if ( !Sql.IsEmptyString(sNAME) && !hashProspectLists.ContainsKey(sNAME) )
										{
											...
											Sql.SetParameter(cmdPROSPECT_LISTS_Import, "@MODIFIED_USER_ID", _security.USER_ID);
							...
							for ( int iRowNumber = 1; i < nlRows.Count ; i++ )
							{
								...
								{
									// CHANGED: Response.IsClientConnected → response.HttpContext.RequestAborted.IsCancellationRequested
									if ( response != null && response.HttpContext.RequestAborted.IsCancellationRequested )
									{
										break;
									}
									...
									Sql.SetParameter(cmdImport, "@MODIFIED_USER_ID", _security.USER_ID);
									...
									// Apply header mappings from the XML row node child elements.
									// CHANGED: hashHeaderMappings (StringDictionary) → hashMappings (Hashtable), same lookup pattern.
									for ( int j = 0; j < node.ChildNodes.Count; j++ )
									{
										string sText = node.ChildNodes[j].InnerText;
										string sName = String.Empty;
										// 08/22/2006 Paul.  We should always use the header mappings instead of an index
										// as nodes may move around.
										sName = Sql.ToString(hashMappings[node.ChildNodes[j].Name]);
									...
									if ( rules != null && dtRules != null && dtRules.Rows.Count > 0 )
									{
										...
										// CHANGED: SplendidImportThis now receives optional Security + IHttpContextAccessor
										SplendidImportThis swThis = new SplendidImportThis(Container, L10n, sImportModule, row, cmdImport, cmdImportCSTM, _security, _httpContextAccessor);
									...
									else
									{
										...
										if ( parID != null )
										{
											...
											if ( sSOURCE == "act" )
											{
												...
												if ( cmdNOTES_Import != null )
												{
													...
													foreach ( XmlNode xNote in nlNotes )
													{
														...
														if ( Sql.IsEmptyString(sNAME) )
															sNAME = "Note";
														// 02/04/2010 Paul.  The modified user is always the person who imported the data.
														Sql.SetParameter(cmdNOTES_Import, "@MODIFIED_USER_ID", _security.USER_ID);
												...
												if ( cmdCALLS_Import != null && cmdMEETINGS_Import != null )
												{
													...
													foreach ( XmlNode xActivity in nlActivities )
													{
														...
														{
															// 02/04/2010 Paul.  The modified user is always the person who imported the data.
															Sql.SetParameter(cmdCALLS_Import, "@MODIFIED_USER_ID", _security.USER_ID );
														...
														{
															// 02/04/2010 Paul.  The modified user is always the person who imported the data.
															Sql.SetParameter(cmdMEETINGS_Import, "@MODIFIED_USER_ID", _security.USER_ID );
												...
												if ( cmdPROSPECT_LISTS_CONTACTS_Import != null )
												{
													...
													foreach ( XmlNode xProspectList in nlProspectLists )
													{
														...
														if ( hashProspectLists.ContainsKey(sNAME) )
														{
															...
															// 02/04/2010 Paul.  The modified user is always the person who imported the data.
															Sql.SetParameter(cmdPROSPECT_LISTS_CONTACTS_Import, "@MODIFIED_USER_ID", _security.USER_ID  );
												...
												if ( cmdPROSPECT_LISTS_LEADS_Import != null )
												{
													...
													foreach ( XmlNode xProspectList in nlProspectLists )
													{
														...
														if ( hashProspectLists.ContainsKey(sNAME) )
														{
															...
															// 02/04/2010 Paul.  The modified user is always the person who imported the data.
															Sql.SetParameter(cmdPROSPECT_LISTS_LEADS_Import, "@MODIFIED_USER_ID", _security.USER_ID  );
												...
												if ( cmdPROSPECT_LISTS_PROSPECTS_Import != null )
												{
													...
													foreach ( XmlNode xProspectList in nlProspectLists )
													{
														...
														if ( hashProspectLists.ContainsKey(sNAME) )
														{
															...
															// 02/04/2010 Paul.  The modified user is always the person who imported the data.
															Sql.SetParameter(cmdPROSPECT_LISTS_PROSPECTS_Import, "@MODIFIED_USER_ID", _security.USER_ID  );
												...
												if ( cmdPROSPECT_LISTS_ACCOUNTS_Import != null )
												{
													...
													foreach ( XmlNode xProspectList in nlProspectLists )
													{
														...
														if ( hashProspectLists.ContainsKey(sNAME) )
														{
															...
															// 02/04/2010 Paul.  The modified user is always the person who imported the data.
															Sql.SetParameter(cmdPROSPECT_LISTS_ACCOUNTS_Import, "@MODIFIED_USER_ID", _security.USER_ID  );
											...
											// 10/27/2017 Paul.  Add Accounts as email source.
											// NOTE: gPROSPECT_LIST_ID is Guid.Empty in the new API (removed from signature).
											// This branch is preserved for future activation.
									...
									// CHANGED: Response.Write(" ") → write heartbeat byte to response body.
									// Null-check required because response may not be available (e.g., background service import).
									if ( response != null )
									{
										try
										{
											byte[] heartbeat = System.Text.Encoding.UTF8.GetBytes(" ");
											response.Body.Write(heartbeat, 0, heartbeat.Length);
										}
										catch { /* suppress heartbeat write errors */ }
									}
								}
								catch(Exception ex)
								{
							...
							// CHANGED: Security.USER_ID (static) → _security.USER_ID (instance)
							string sProcessedFileName = _security.USER_ID.ToString() + " " + Guid.NewGuid().ToString() + ".xml";
							...
							// CHANGED: Session["TempFile." + sProcessedFileID] = sProcessedFileName →
							//          session.SetString("TempFile." + id, sProcessedFileName) for each ID in array.
							if ( session != null && sProcessedFileIDs != null )
							{
								foreach (string sProcessedFileID in sProcessedFileIDs)
								{
									if ( !Sql.IsEmptyString(sProcessedFileID) )
										session.SetString("TempFile." + sProcessedFileID, sProcessedFileName);
								}
							}
