 ...
 *********************************************************************************************************************/
// .NET 10 Migration: SplendidCRM/_code/SplendidImport.cs → src/SplendidCRM.Core/SplendidImport.cs
// Changes applied:
//   - REMOVED: using System.Web; (HttpContext.Current static access)
//   - REMOVED: using System.Data.OleDb; (OleDb path was in dead #else branch — never compiled)
//   - ADDED:   using Microsoft.AspNetCore.Http; using Microsoft.Extensions.Caching.Memory;
//   - Static class pattern → DI instance class with constructor:
//       IHttpContextAccessor httpContextAccessor
//       IMemoryCache         memoryCache
//       DbProviderFactories  dbProviderFactories
//       Security             security
//   - HttpContext.Current.Response        → _httpContextAccessor.HttpContext?.Response
//   - Response.IsClientConnected          → !(_httpContextAccessor.HttpContext?.RequestAborted.IsCancellationRequested ?? false)
//   - Response.Write(" ")                 → REMOVED — legacy IIS keep-alive; Kestrel handles HTTP keep-alive at transport level
//   - HttpContext.Current.Session["key"]  → _httpContextAccessor.HttpContext?.Session?.SetString("key", value)
//   - Security.USER_ID  (static)          → _security.USER_ID  (instance property)
//   - Security.TEAM_ID  (static)          → _security.TEAM_ID  (instance property)
//   - DbProviderFactories.GetFactory()    → _dbProviderFactories.GetFactory() (DI instance)
//   - Crm.Config.*()                      → preserved (static ambient methods)
//   - SplendidError.SystemWarning()       → preserved (static method)
//   - SqlProcs.Factory()                  → preserved (static method)
//   - Sql.CreateDbName(IDbCommand, "@"+n) → Sql.CreateDbName("@"+n) — single-arg; SQL Server no conversion needed
//   - Sql.ExtractDbName(IDbCommand, par.ParameterName) → par.ParameterName.TrimStart('@') inline
//       MIGRATION NOTE: migrated Sql.ExtractDbName(string) extracts schema prefix from table names,
//       not provider-specific param prefixes.  For SQL Server the inline @ strip is equivalent.
//   - Methods accessing HttpContext converted from static to instance (Import, ConvertDBaseToXml, ConvertStreamToXml)
//   - Pure helper methods remain static-compatible (ZipSaveStream, ConvertTableToXml, ConvertTextToXml, ConvertXmlSpreadsheetToXml)
//   - Dead #else OleDb branch of ConvertDBaseToXml removed (was inside #if false — never executed)
#nullable disable
using System;
using System.IO;
using System.Data;
...
{
	/// <summary>
	/// Data import processing for SplendidCRM.
	/// Supports import of CSV, tab-delimited, XML, Excel XML Spreadsheet, Excel 2007 XLSX,
	/// legacy Excel XLS, dBase (.dbf via ODBC), and ACT! formats.
	///
	/// Migrated from SplendidCRM/_code/SplendidImport.cs for .NET 10 ASP.NET Core.
	/// Replaces System.Web dependencies (HttpContext.Current, Response, Session) with
	/// ASP.NET Core DI equivalents (IHttpContextAccessor, IMemoryCache).
	/// </summary>
	public class SplendidImport
	{
		// =====================================================================================
		// DI fields — injected via constructor
		// BEFORE: static access to HttpContext.Current (System.Web)
		// AFTER:  constructor-injected IHttpContextAccessor (Microsoft.AspNetCore.Http)
		// =====================================================================================
		...
		/// <summary>
		/// Constructs a SplendidImport instance with required DI services.
		/// </summary>
		/// <param name="httpContextAccessor">
		///   Replaces HttpContext.Current: used in Import() to check client connection
		///   and in ConvertDBaseToXml() to track temp files in distributed session.
		/// </param>
		/// <param name="memoryCache">
		///   Replaces Application[] state access pattern used by dependency services.
		/// </param>
		/// <param name="dbProviderFactories">
		///   Replaces DbProviderFactories.GetFactory() static call in Import().
		/// </param>
		/// <param name="security">
		///   Replaces Security.USER_ID / Security.TEAM_ID static access in Import().
		/// </param>
		public SplendidImport(
			IHttpContextAccessor httpContextAccessor,
			IMemoryCache         memoryCache,
			DbProviderFactories  dbProviderFactories,
			Security             security)
		...
		/// <summary>
		/// Appends an HTML-formatted error entry to the error accumulator string builder.
		/// Called by Import() to collect per-row and per-table errors for final reporting.
		/// </summary>
		...
		/// <summary>
		/// Returns true when the current HTTP request has not been cancelled by the client.
		/// Replaces Response.IsClientConnected from System.Web which is not available in ASP.NET Core.
		/// Falls back to true (continue processing) when no HTTP context is available
		/// (e.g. when called from a background service or unit test).
		/// </summary>
		private bool IsClientConnected
		{
			get
			{
				// .NET 10 Migration: Response.IsClientConnected → check RequestAborted CancellationToken
				// If no HttpContext (background call), default to true (continue processing).
				return !(_httpContextAccessor.HttpContext?.RequestAborted.IsCancellationRequested ?? false);
			}
		}
		...
		/// <summary>
		/// Imports records from an XML document into the database.
		/// Iterates over all table nodes in the XML, optionally truncating existing data first,
		/// then inserts records either via raw INSERT statements (non-reserved tables) or via
		/// spXXX_Update stored procedures (reserved configuration tables).
		///
		/// BEFORE (static): public static void Import(XmlDocument xml, ArrayList arrTables, bool bTruncate)
		/// AFTER  (instance): uses _dbProviderFactories, _security, and _httpContextAccessor via DI
		/// </summary>
		/// <param name="xml">The XML document containing table data to import.</param>
		/// <param name="arrTables">Ordered list of table names to process; null to load from vwSqlTableDependencies.</param>
		/// <param name="bTruncate">Whether to truncate (delete) existing records before importing.</param>
		/// <exception cref="Exception">Thrown with accumulated HTML error report if any rows failed to import.</exception>
		public void Import(XmlDocument xml, ArrayList arrTables, bool bTruncate)
		{
			...
			// 04/19/2010 Paul.  Add separate table for EditView Relationships. 
			...
			// .NET 10 Migration: DbProviderFactories.GetFactory() static → _dbProviderFactories.GetFactory() instance
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						if ( bTruncate )
						{
							...
							using ( DbDataAdapter da = dbf.CreateDataAdapter() )
							{
								...
								{
									da.Fill(dt);
									// .NET 10 Migration: Response.IsClientConnected → IsClientConnected property
									for ( int i = 0 ; i < dt.Rows.Count && IsClientConnected ; i++ )
									{
										DataRow row = dt.Rows[i];
										...
										if ( hashTables.ContainsKey(sTABLE_NAME) || hashReferenced.ContainsKey(sTABLE_NAME) )
										{
											try
											{
												...
												cmd.ExecuteNonQuery();
												// .NET 10 Migration: Response.Write(" ") REMOVED — legacy IIS keep-alive technique.
												// In ASP.NET Core + Kestrel, HTTP keep-alive is managed at the transport/protocol level.
#if DEBUG
			...
			// .NET 10 Migration: Response.IsClientConnected → IsClientConnected property
			for ( int i = 0 ; i < arrTables.Count && IsClientConnected ; i++ )
			{
										...
										using ( IDbCommand cmdImport = con.CreateCommand() )
										{
												...
												if ( vwColumns.Count == 1 )
												{
													...
													if ( Sql.FindParameter(cmdImport, "@" + sName) == null )
													{
														...
														if ( cmdImport.CommandText.Length > 0 )
															cmdImport.CommandText += ", ";
														// .NET 10 Migration: Sql.CreateDbName(cmdImport, "@"+sName) → Sql.CreateDbName("@"+sName)
														// For SQL Server the single-arg version returns the name unchanged (no @ → : conversion needed).
														cmdImport.CommandText += Sql.CreateDbName("@" + sName);
											...
											if ( nRowErrors == 0 )
											{
												try
												{
													...
													// .NET 10 Migration: Response.Write(" ") REMOVED — legacy IIS keep-alive.
												}
												catch(Exception ex)
												{
							...
							try
							{
								...
								foreach(XmlNode node in nlRows)
								{
									...
									foreach(IDbDataParameter par in cmdImport.Parameters)
									{
										...
										// .NET 10 Migration: Sql.ExtractDbName(cmdImport, par.ParameterName) →
										//   inline @ stripping. Sql.ExtractDbName(string) in migrated Sql.cs
										//   extracts schema prefix from "schema.table" format (not param names).
										//   For SQL Server, par.ParameterName is "@PARAM_NAME"; stripping "@" gives "PARAM_NAME".
										...
										if ( sParameterName == "TEAM_ID" && bEnableTeamManagement ) // 02/26/2011 Paul.  Ignore the Required flag. && bRequireTeamManagement )
											par.Value = Sql.ToDBGuid(_security.TEAM_ID);  // 02/26/2011 Paul.  Make sure to convert Guid.Empty to DBNull. 
										else if ( sParameterName == "ASSIGNED_USER_ID" ) // 02/26/2011 Paul.  Always set the Assigned User ID. && bRequireUserAssignment )
											par.Value = Sql.ToDBGuid(_security.USER_ID);  // 02/26/2011 Paul.  Make sure to convert Guid.Empty to DBNull. 
		...
		// =====================================================================================
		// Static helper methods — no HttpContext dependency; can be called without an instance
		// These remain public static to preserve call sites that may invoke them directly.
		// =====================================================================================

		/// <summary>
		/// Copies the contents of a stream to a file at the specified path using binary I/O.
		/// Used to write uploaded ZIP-stream contents (e.g. dBase files) to a temp file
		/// before ODBC processing.
		/// </summary>
		/// <param name="stm">Source stream to copy from (e.g. decompressed ZIP stream entry).</param>
		/// <param name="sTempPathName">Destination file path to write to.</param>
		...
		/// <summary>
		/// Converts a DataTable to an XML import document using generic field names (ImportField000, etc.).
		/// Overload without header row and column name override.
		/// </summary>
		/// <param name="dt">Source DataTable.</param>
		/// <param name="sRecordName">XML element name for each data row.</param>
		/// <returns>XmlDocument suitable for field-mapping import dialogs.</returns>
		...
		/// <summary>
		/// Converts a DataTable to an XML import document.
		/// Optionally includes a header row and uses column names directly as field names.
		/// Columns listed in <paramref name="arrIgnoreColumns"/> are excluded.
		/// </summary>
		/// <param name="dt">Source DataTable.</param>
		/// <param name="sRecordName">XML element name for each data row.</param>
		/// <param name="bWithHeader">If true, prepends a header record containing column names as ImportField values.</param>
		/// <param name="bUseColumnName">If true, uses the DataColumn name as the XML element name; otherwise uses ImportField000 format.</param>
		/// <param name="arrIgnoreColumns">Optional list of column names to skip during conversion.</param>
		/// <returns>XmlDocument suitable for field-mapping import dialogs.</returns>
		...
		/// <summary>
		/// Reads a dBase (.dbf) file from a stream and converts it to an XML import document.
		/// Overload without additional columns list.
		/// </summary>
		/// <param name="stm">Stream containing the dBase (.dbf) file content.</param>
		/// <param name="sRecordName">XML element name for each data row.</param>
		/// <param name="bUseColumnName">If true, uses column names directly as XML element names.</param>
		/// <param name="arrIgnoreColumns">Optional list of column names to skip.</param>
		/// <returns>XmlDocument suitable for field-mapping import dialogs.</returns>
		public XmlDocument ConvertDBaseToXml(Stream stm, string sRecordName, bool bUseColumnName, List<String> arrIgnoreColumns)
		{
		...
		/// <summary>
		/// Reads a dBase (.dbf) file from a stream and converts it to an XML import document.
		/// Writes the stream to a temp file, opens it via the Microsoft dBase ODBC driver,
		/// and loads the data into a DataTable for XML conversion.
		///
		/// BEFORE (static): public static XmlDocument ConvertDBaseToXml(...)
		/// AFTER  (instance): accesses distributed session via _httpContextAccessor to track temp file path
		///
		/// MIGRATION NOTE: The ODBC dBase driver (Microsoft dBase Driver *.dbf) is Windows-only.
		/// On Linux/Kestrel, calling this method will throw an exception. This is expected behavior
		/// since dBase import was Windows-only in the legacy IIS deployment as well.
		/// </summary>
		/// <param name="stm">Stream containing the dBase (.dbf) file content.</param>
		/// <param name="sRecordName">XML element name for each data row.</param>
		/// <param name="bUseColumnName">If true, uses column names directly as XML element names.</param>
		/// <param name="arrIgnoreColumns">Optional list of column names to remove before conversion.</param>
		/// <param name="arrAdditionalColumns">Optional list of additional empty string columns to append.</param>
		/// <returns>XmlDocument suitable for field-mapping import dialogs.</returns>
		public XmlDocument ConvertDBaseToXml(Stream stm, string sRecordName, bool bUseColumnName, List<String> arrIgnoreColumns, List<String> arrAdditionalColumns)
		{
			XmlDocument xml = new XmlDocument();
			xml.AppendChild(xml.CreateProcessingInstruction("xml" , "version=\"1.0\" encoding=\"UTF-8\""));
			xml.AppendChild(xml.CreateElement("xml"));
			...
			// .NET 10 Migration: HttpContext.Current.Session["TempFile."+sTempFileID] = sTempPathName
			// → _httpContextAccessor.HttpContext?.Session?.SetString() (distributed session)
			// Session stores temp file path so upload cleanup can locate and delete the temp file.
			_httpContextAccessor.HttpContext?.Session?.SetString("TempFile." + sTempFileID, sTempPathName);
		...
		/// <summary>
		/// Parses a text stream (CSV or tab-delimited) into an XML import document.
		/// Handles both quoted and unquoted fields, with support for escaped double quotes ("").
		/// Ensures the first record (used for header mapping) has all detected field positions.
		///
		/// BEFORE (static): public static XmlDocument ConvertTextToXml(...)
		/// AFTER  (static): no HttpContext dependency; remains static.
		/// </summary>
		/// <param name="sRecordName">XML element name for each parsed line.</param>
		/// <param name="stm">Input stream containing the delimited text content.</param>
		/// <param name="chFieldSeparator">Field delimiter character (comma, tab, or custom).</param>
		/// <returns>XmlDocument with ImportField000-indexed elements per field.</returns>
		...
		/// <summary>
		/// Converts an Excel XML Spreadsheet (Excel 2003 "xmlspreadsheet" format) to an XML import document.
		/// Handles missing cells (sparse rows) by inserting empty ImportField elements for gaps.
		/// Uses the "urn:schemas-microsoft-com:office:spreadsheet" namespace (ss:) for XPath queries.
		/// </summary>
		/// <param name="xml">XmlDocument containing the Excel XML Spreadsheet content.</param>
		/// <param name="sRecordName">XML element name for each spreadsheet row.</param>
		/// <returns>XmlDocument with ImportField000-indexed elements per column.</returns>
		...
		/// <summary>
		/// Detects the source format from <paramref name="sSourceType"/> and converts the input stream
		/// to a SplendidCRM XML import document ready for field mapping.
		///
		/// Supported formats:
		///   xmlspreadsheet — Excel XML Spreadsheet (Excel 2003 .xml export)
		///   xml            — SplendidCRM XML export (or auto-detected Excel Sheet XML)
		///   excel          — Excel XLSX (OpenXML, via Excel2007Reader) or XLS (BIFF, via ExcelDataReader)
		///   other_tab      — Tab-delimited text
		///   custom_delimited — Custom single-character delimiter
		///   dbase          — dBase III/IV (.dbf file, via Microsoft dBase ODBC driver)
		///   act            — ACT! dBase export (via ACTImport)
		///   (default)      — Comma-separated CSV
		///
		/// BEFORE (static): public static XmlDocument ConvertStreamToXml(...)
		/// AFTER  (instance): calls instance ConvertDBaseToXml() for dbase/act paths which require session access
		/// </summary>
		/// <param name="sImportModule">Module name (e.g. "Contacts") used as the XML record element name.</param>
		/// <param name="sSourceType">Source format identifier string (see supported formats above).</param>
		/// <param name="sCustomDelimiterValue">Custom delimiter string (used only for custom_delimited; defaults to comma if empty).</param>
		/// <param name="stm">Input stream containing the import file data.</param>
		/// <param name="sFILE_EXT">File extension including dot (e.g. ".xlsx") for Excel format disambiguation.</param>
		/// <returns>XmlDocument with ImportField000-indexed elements ready for the field-mapping import dialog.</returns>
		public XmlDocument ConvertStreamToXml(string sImportModule, string sSourceType, string sCustomDelimiterValue, Stream stm, string sFILE_EXT)
		{
			...
			switch ( sSourceType )
			{
				...
				case "dbase":
				{
					...
					// .NET 10 Migration: ConvertDBaseToXml() is now an instance method (requires session access for temp file tracking)
