 ...
 *********************************************************************************************************************/
// .NET 10 Migration: SplendidCRM/_code/SplendidExport.cs → src/SplendidCRM.Core/SplendidExport.cs
// Changes applied:
//   - REMOVED: using System.Web; (System.Web.HttpContext.Current → IHttpContextAccessor via DI)
//   - ADDED:   using Microsoft.AspNetCore.Http; using Microsoft.Extensions.Caching.Memory;
//   - Static class → DI-friendly instance class with constructor-injected services
//   - All HttpContext.Current.Session["key"] → IHttpContextAccessor.HttpContext?.Session.GetString("key")
//   - All HttpContext.Current.Application["key"] → IMemoryCache.Get<T>("key")
//   - HttpContext.Current.Response → IHttpContextAccessor.HttpContext?.Response
//   - Response.OutputStream → HttpContext.Response.Body
//   - Response.AddHeader() → HttpContext.Response.Headers["Content-Disposition"] = 
//   - Response.End() → removed (controller handles response lifecycle in ASP.NET Core)
//   - Response.ClearContent()/ClearHeaders() → ContentType reset only (streaming cannot be reversed)
//   - CalendarControl.SqlDateTimeFormat (WebForms type) → local constant "yyyy-MM-dd HH:mm:ss"
//   - new L10N(culture) → new L10N(culture, _memoryCache) (updated constructor signature)
//   - SplendidCache.CustomList(name, value, ref bool) → _splendidCache.CustomList(name, culture) DataTable lookup
//   - SplendidCache.AssignedUser(Guid) → _splendidCache.AssignedUser(Guid) DataTable → FULL_NAME row extraction
//   - Utils.ContentDispositionEncode(browser, filename) → Utils.ContentDispositionEncode(filename)
//   - Static public methods → public instance methods (private static OpenXML helpers remain static)
//   - GetExportTranslatedLists → instance method (accesses _memoryCache, _splendidCache, _security)
//   - TranslateExportedvalue → instance method (accesses _splendidCache)
...
{
	/// <summary>
	/// Data export to CSV, Excel (.xlsx OpenXML), Excel (.xls SpreadsheetML), XML, and tab-delimited formats.
	/// Migrated from SplendidCRM/_code/SplendidExport.cs for .NET 10 ASP.NET Core.
	///
	/// DESIGN NOTES:
	///   • Register as a SCOPED service (per-request) so it inherits the correct IHttpContextAccessor scope.
	///   • Export() writes directly to HttpContext.Response.Body and sets Content-Type/Content-Disposition headers.
	///   • ExportExcelOpenXML/ExportExcel/ExportXml/ExportDelimited accept an explicit Stream and can be
	///     called from controller actions that manage the response lifecycle themselves.
	///   • All private static OpenXML helper methods have no instance-state dependencies and remain static.
	/// </summary>
	public class SplendidExport
	{
		// =====================================================================================
		// Private DI-injected instance fields
		// BEFORE: accessed via static HttpContext.Current and SplendidCache/Security/SplendidDynamic statics
		// AFTER:  constructor-injected DI services
		// =====================================================================================
		...
		// .NET 10 Migration: CalendarControl.SqlDateTimeFormat is a System.Web.UI type not available in .NET 10.
		// Replacing with the identical ISO 8601 combined date/time format string.
		...
		/// <summary>
		/// Constructs a SplendidExport instance with all required DI services injected.
		/// Register this class as SCOPED in the DI container to ensure per-request HttpContext access.
		/// </summary>
		/// <param name="httpContextAccessor">Replaces HttpContext.Current throughout</param>
		/// <param name="memoryCache">Replaces Application[] global state throughout</param>
		/// <param name="security">Provides FULL_NAME and PRIMARY_ROLE_NAME session properties</param>
		/// <param name="splendidCache">Provides GridViewColumns, CustomList, and AssignedUser metadata</param>
		/// <param name="splendidDynamic">Provides GridColumns for export field resolution</param>
		public SplendidExport(
		...
		// =====================================================================================
		// Private helper: resolve request-scoped L10N instance
		// BEFORE: L10N L10n = HttpContext.Current.Items["L10n"] as L10N;
		// AFTER:  L10N L10n = _httpContextAccessor.HttpContext?.Items["L10n"] as L10N;
		// Falls back to session culture or "en-US" default when Items["L10n"] is not populated.
		// =====================================================================================
		private L10N GetCurrentL10n()
		{
			// .NET 10 Migration: HttpContext.Current.Items["L10n"] → _httpContextAccessor.HttpContext?.Items["L10n"]
			L10N L10n = _httpContextAccessor.HttpContext?.Items["L10n"] as L10N;
			// 12/15/2019 Paul.  L10n will be null for Rest service call.
			if ( L10n == null )
			{
				// .NET 10 Migration: HttpContext.Current.Session["USER_SETTINGS/CULTURE"]
				//                  → _httpContextAccessor.HttpContext?.Session?.GetString("USER_SETTINGS/CULTURE")
				string sCulture = _httpContextAccessor.HttpContext?.Session?.GetString("USER_SETTINGS/CULTURE");
				if ( Sql.IsEmptyString(sCulture) )
					sCulture = "en-US";
				// .NET 10 Migration: new L10N(culture) → new L10N(culture, _memoryCache) (updated constructor)
		...
		// Sets response Content-Type/Content-Disposition headers, streams export content to
		// HttpContext.Response.Body, and dispatches to the appropriate format method.
		//
		// BEFORE: public static void Export(DataView, string, string, string, int, int, string[], bool)
		// AFTER:  public void Export(...)  — instance method using _httpContextAccessor for response access
		// =====================================================================================
		...
		public void Export(DataView vw, string sModuleName, string sExportFormat, string sExportRange, int nCurrentPage, int nPageSize, string[] arrID, bool bCustomPaging)
		{
			...
			// .NET 10 Migration: HttpContext.Current.Response → _httpContextAccessor.HttpContext.Response
			Microsoft.AspNetCore.Http.HttpResponse Response = _httpContextAccessor.HttpContext?.Response;
			...
			try
			{
				switch ( sExportFormat )
				{
					case "csv"  :
						...
						// .NET 10 Migration: Response.AddHeader() → Response.Headers["Content-Disposition"] =
						// Utils.ContentDispositionEncode(browser, filename) → Utils.ContentDispositionEncode(filename)
						Response.Headers["Content-Disposition"] = "attachment;filename=" + Utils.ContentDispositionEncode(sModuleName + ".csv");
						// .NET 10 Migration: Response.OutputStream → Response.Body
						ExportDelimited(Response.Body, vw, sModuleName, nStartRecord, nEndRecord, ',' );
						// .NET 10 Migration: Response.End() removed — controller handles response lifecycle in ASP.NET Core
						break;
					case "tab"  :
						// 08/17/2024 Paul.  The correct MIME type is text/plain. 
						Response.ContentType = "text/plain";
						Response.Headers["Content-Disposition"] = "attachment;filename=" + Utils.ContentDispositionEncode(sModuleName + ".txt");
						ExportDelimited(Response.Body, vw, sModuleName, nStartRecord, nEndRecord, '\t');
						break;
					case "xml"  :
						Response.ContentType = "text/xml";
						Response.Headers["Content-Disposition"] = "attachment;filename=" + Utils.ContentDispositionEncode(sModuleName + ".xml");
					...
					default     :
						...
						Response.Headers["Content-Disposition"] = "attachment;filename=" + Utils.ContentDispositionEncode(sModuleName + ".xlsx");
			...
			{
				// .NET 10 Migration: ThreadAbortException cannot be raised in .NET 5+ (Thread.Abort() throws PlatformNotSupportedException).
				// This check is preserved for compatibility but is effectively always true on .NET 10.
				if ( !(ex is System.Threading.ThreadAbortException) )
				{
					// 11/29/2013 Paul.  In case of exception, try and clear the response type.
					// .NET 10 Migration: Response.ClearContent()/ClearHeaders() not available in ASP.NET Core.
					// Reset ContentType to text/html; partial body content cannot be cleared after streaming starts.
					if ( Response != null )
		...
		// =====================================================================================
		// Private static OpenXML helper methods
		// These have no instance-state dependencies and remain static for performance.
		// =====================================================================================
		...
		{
			// 12/02/2013 Paul.  SharedString got very slow after 2000 records.
			...
			cell.InlineString = new DocumentFormat.OpenXml.Spreadsheet.InlineString { Text = new DocumentFormat.OpenXml.Spreadsheet.Text { Text = sText } };
			return cell;
		}
		...
		private static Stylesheet OpenXML_CreateStylesheet()
		{
			...
			var fonts       = new Fonts      (new OpenXmlElement[] { new Font      () }) { Count = 1 };
			var fills       = new Fills      (new OpenXmlElement[] { new Fill      () }) { Count = 1 };
			var borders     = new Borders    (new OpenXmlElement[] { new Border    () }) { Count = 1 };
			var cellFormats = new CellFormats(new OpenXmlElement[] { new CellFormat() }) { Count = 1 };
		...
		// =====================================================================================
		// Private instance helpers — require access to injected DI services
		// =====================================================================================

		// 09/08/2020 Paul.  Some customers want exported lists to be translated. 
		// .NET 10 Migration: static → instance method (accesses _memoryCache, _splendidCache, _security)
		private Dictionary<string, string> GetExportTranslatedLists(string sModuleName)
		{
			Dictionary<string, string> dict = new Dictionary<string, string>();
			// .NET 10 Migration: Context != null && Context.Application["CONFIG.export_translate_lists"]
			//                  → _memoryCache.Get<object>("CONFIG.export_translate_lists")
			// Application state is always available via IMemoryCache; no null context check required.
			if ( Sql.ToBoolean(_memoryCache.Get<object>("CONFIG.export_translate_lists")) )
			{
				// .NET 10 Migration: SplendidCache.GridViewColumns(name, Security.PRIMARY_ROLE_NAME)
				//                  → _splendidCache.GridViewColumns(name, _security.PRIMARY_ROLE_NAME)
				DataTable dt = _splendidCache.GridViewColumns(sModuleName + ".Export", _security.PRIMARY_ROLE_NAME);
				if ( dt != null )
				{
		...
		// .NET 10 Migration: static → instance method (accesses _splendidCache for custom list and user lookups)
		private string TranslateExportedvalue(Dictionary<string, string> dict, L10N L10n, string sDATA_FIELD, DataRowView row)
		{
			...
			if ( dict.ContainsKey(sDATA_FIELD) )
			{
				...
				if ( !Sql.IsEmptyString(sLIST_NAME) )
				{
					...
					// .NET 10 Migration: SplendidCache.CustomList(sLIST_NAME, sValue, ref bCustomCache) 
					// → _splendidCache.CustomList(sLIST_NAME, sCULTURE) returning a DataTable.
					// For data lists (Currencies, Shippers, etc.) in the CustomCaches registry,
					// we look up the value in the table's ID column and return the matching display name.
					// This replicates the original bCustomCache = true early-return path.
					string sCULTURE = L10n?.NAME ?? "en-US";
					DataTable dtCustom = _splendidCache.CustomList(sLIST_NAME, sCULTURE);
					if ( dtCustom != null && dtCustom.Rows.Count > 0 )
					{
						// Identify the ID column and display name column in the custom list DataTable.
						// AssignedUser: ID (Guid), FULL_NAME. Currencies: ID, NAME. Others: varies.
						string sIdCol = dtCustom.Columns.Contains("ID") ? "ID" : dtCustom.Columns[0].ColumnName;
						string sDisplayCol = dtCustom.Columns.Contains("FULL_NAME")    ? "FULL_NAME"
						                   : dtCustom.Columns.Contains("DISPLAY_NAME") ? "DISPLAY_NAME"
						                   : dtCustom.Columns.Contains("NAME")         ? "NAME"
						                   : (dtCustom.Columns.Count > 1 ? dtCustom.Columns[1].ColumnName : dtCustom.Columns[0].ColumnName);
						foreach ( DataRow r in dtCustom.Rows )
						{
							// Case-insensitive comparison handles Guid string representations.
							if ( string.Equals(Sql.ToString(r[sIdCol]), sValue, StringComparison.OrdinalIgnoreCase) )
							{
								return Sql.ToString(r[sDisplayCol]);
							}
						}
					}
					...
					{
						// .NET 10 Migration: SplendidCache.AssignedUser(Guid) now returns DataTable with ID/FULL_NAME columns.
						// Extract FULL_NAME from the first matching row.
						DataTable dtUser = _splendidCache.AssignedUser(Sql.ToGuid(row[sDATA_FIELD]));
						if ( dtUser != null && dtUser.Rows.Count > 0 )
							sValue = Sql.ToString(dtUser.Rows[0]["FULL_NAME"]);
						return sValue;
					}
		...
		// =====================================================================================
		// Public export format methods
		// BEFORE: public static void ExportExcelOpenXML(...)
		// AFTER:  public void ExportExcelOpenXML(...)  — instance method
		// =====================================================================================
		...
		public void ExportExcelOpenXML(Stream stmResponse, DataView vw, string sModuleName, int nStartRecord, int nEndRecord)
		{
			...
			// .NET 10 Migration: SplendidDynamic.GridColumns(...) → _splendidDynamic.GridColumns(...)
			_splendidDynamic.GridColumns(sModuleName + ".Export", arrExportFields, null);
			...
			// .NET 10 Migration: L10N L10n = HttpContext.Current.Items["L10n"] as L10N → GetCurrentL10n()
			L10N L10n = GetCurrentL10n();

			// 05/10/2016 Paul.  The User Primary Role is used with role-based views. 
			// .NET 10 Migration: SplendidCache.GridViewColumns(name, Security.PRIMARY_ROLE_NAME)
			//                  → _splendidCache.GridViewColumns(name, _security.PRIMARY_ROLE_NAME)
			DataTable dtExport = _splendidCache.GridViewColumns(sModuleName + ".Export", _security.PRIMARY_ROLE_NAME);
			...
			using ( MemoryStream stm = new MemoryStream() )
			{
				using ( SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(stm, SpreadsheetDocumentType.Workbook) )
				{
					...
					workbookPart.Workbook.Save();
					// .NET 10 Migration: SpreadsheetDocument.Close() removed in OpenXML SDK 3.x.
					// The using block disposes the document automatically — explicit Close() not needed.
				}
		...
		public void ExportExcel(Stream stm, DataView vw, string sModuleName, int nStartRecord, int nEndRecord)
		{
			...
			xw.WriteStartElement("DocumentProperties");
				...
				xw.WriteStartElement("Author");
					// .NET 10 Migration: Security.FULL_NAME (static) → _security.FULL_NAME (instance property)
					xw.WriteString(_security.FULL_NAME);
				xw.WriteEndElement();
				...
				for ( int nFieldIndex = 0; nFieldIndex < arrValidatedFields.Count; nFieldIndex++ )
				{
					...
					if ( row[sField] != DBNull.Value )
					{
						switch ( col.DataType.FullName )
						{
							...
							case "System.DateTime":
								// .NET 10 Migration: CalendarControl.SqlDateTimeFormat → local constant SqlDateTimeFormat
								xw.WriteString(Sql.ToDateTime(row[sField]).ToUniversalTime().ToString(SqlDateTimeFormat));
								break;
				...
				for ( int nFieldIndex = 0; nFieldIndex < arrValidatedFields.Count; nFieldIndex++ )
				{
					...
					if ( row[sField] != DBNull.Value )
					{
						...
						switch ( col.DataType.FullName )
						{
							...
							case "System.DateTime":
								// .NET 10 Migration: CalendarControl.SqlDateTimeFormat → local constant SqlDateTimeFormat
								sValue = Sql.ToDateTime(row[sField]).ToUniversalTime().ToString(SqlDateTimeFormat);
								break;
