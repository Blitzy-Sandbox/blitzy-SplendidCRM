...
// .NET 10 Migration: Spring.Social.OAuth2 dependencies replaced with standalone stubs.
// Spring.Json.JsonValue replaced with System.Text.Json.JsonElement.
// Spring.Http.HttpUtils.UrlEncode replaced with System.Net.WebUtility.UrlEncode.
...
namespace Spring.Social.Salesforce.Connect
{
	...
	public class SalesforceServiceProvider
	{
		...
		private readonly SalesforceOAuth2Template oAuth2Template;
		...
		public SalesforceServiceProvider(string clientId, string clientSecret)
		{
			...
			this.oAuth2Template = new SalesforceOAuth2Template(clientId, clientSecret);
		}
		...
		public ISalesforce GetApi(string accessToken)
		{
		...
		public static JsonElement GetAccessToken(string clientId, string clientSecret, string username, string password, string securityToken)
		{
			...
			string request = String.Format("grant_type=password&client_id={0}&client_secret={1}&username={2}&password={3}&format=json", WebUtility.UrlEncode(clientId), WebUtility.UrlEncode(clientSecret), WebUtility.UrlEncode(username), WebUtility.UrlEncode(password + securityToken));
			...
			byte[] by = Encoding.UTF8.GetBytes(request);
				...
				if ( objResponse != null )
				{
					...
					else
					{
						using ( StreamReader readStream = new StreamReader(objResponse.GetResponseStream(), Encoding.UTF8) )
						{
							...
							using (JsonDocument doc = JsonDocument.Parse(sResponse)) { jsonElement = doc.RootElement.Clone(); }
						}
					}
				}
			}
