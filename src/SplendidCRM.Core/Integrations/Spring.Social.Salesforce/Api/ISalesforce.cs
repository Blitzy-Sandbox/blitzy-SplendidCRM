...
// Migration Note: .NET 10 ASP.NET Core replatforming — Spring.Rest.Client removed (no .NET 10 equivalent).
// IApiBinding and IRestOperations are defined as local stubs below to preserve public contract for
// dormant Enterprise Edition integration stubs. These stubs must compile but NOT execute.
using System;

namespace Spring.Social.Salesforce.Api
{
	// Stub interface replacing Spring.Rest.Client.IApiBinding (dormant integration stub)
	// Preserves the public contract for SalesforceOAuth2ApiBinding in Impl/ to compile against.
	public interface IApiBinding
	{
		bool IsAuthorized { get; }
	}

	// Stub interface replacing Spring.Rest.Client.IRestOperations (dormant integration stub)
	// Preserves the public contract for AbstractSalesforceOperations in Impl/ to compile against.
