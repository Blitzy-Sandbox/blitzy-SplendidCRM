...
// .NET 10 Migration: Removed "using Spring.Json;" — Spring.JSON types (IJsonDeserializer, JsonValue,
// JsonMapper) are unavailable on .NET 10 (discontinued Spring.NET Framework assemblies with no NuGet
// equivalent). Stub replacements are defined in the parent namespace Spring.Social.Salesforce.Api.Impl
// (AbstractSalesforceOperations.cs) and are accessible here via C# namespace resolution without an
// explicit using directive. This is a dormant integration stub — compiles but is NOT expected to
// execute at runtime (AAP section 0.7.4).
