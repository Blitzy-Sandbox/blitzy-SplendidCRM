...
// .NET 10 Migration: Removed "using Spring.Json;" — Spring.NET Framework assemblies are discontinued
// with no .NET Core / .NET 10 equivalent NuGet packages. The replacement stub types IJsonDeserializer,
// JsonValue, and JsonMapper are defined in the parent namespace Spring.Social.Salesforce.Api.Impl
// (AbstractSalesforceOperations.cs) and resolve via C# namespace resolution without an explicit using
// directive. This is a dormant integration stub — compiles but is NOT expected to execute at runtime.
