...
// Migrated from SplendidCRM/_code/Spring.Social.Facebook/Api/Impl/MediaTemplate.cs
...
//                                         JsonValue resolves from stub in AbstractFacebookOperations.cs
//   - REMOVED: using Spring.Http;       — Spring.Http has no .NET 10 NuGet equivalent;
//                                         Resource resolves to local Spring.Social.Facebook.Api.Resource DTO
//   - REMOVED: using Spring.Rest.Client; — Spring.Rest DLL has no .NET 10 NuGet equivalent;
//                                          RestTemplate resolves from stub in AbstractFacebookOperations.cs
//   - ADDED:   using Spring.Social.Facebook.Api; — resolves Album, Photo, Video, Resource,
//                                                  ImageType, IMediaOperations from parent namespace
//   - CHANGED: class access modifier from internal (implicit) to public — required for class library
//              export where base class AbstractFacebookOperations is declared public. Per AAP §0.8.1:
//              minimal migration-required change only.
//   - KEPT:    All method signatures and implementations preserved exactly from source.
//              restTemplate.PostForObject<JsonValue>() in PostVideo() resolves to stubs defined
//              in AbstractFacebookOperations.cs (single definition point for the Impl/ namespace).
