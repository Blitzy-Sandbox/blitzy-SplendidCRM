...
// MIGRATION: Removed 'using Spring.Rest.Client;' — Spring.Rest is a discontinued .NET Framework library
// with no .NET 10 equivalent. Types referenced only in XML doc cref attributes (ApiException,
// InsufficientPermissionException, MissingAuthorizationException) do not affect compilation.
// MIGRATION: Removed 'using Spring.Http;' — not used in actual interface code, only in doc comments.
// Per AAP 0.8.1 Minimal Change Clause: this is a dormant Enterprise Edition stub that must compile
// on .NET 10 but is not expected to execute.
