...
// NOTE: .NET 10 Migration — replaced 'using System.Web;' with 'using Microsoft.AspNetCore.Http;'
// System.Web is not available in .NET 10 / ASP.NET Core. HttpContext now refers to Microsoft.AspNetCore.Http.HttpContext.
using Microsoft.AspNetCore.Http;
// NOTE: .NET 10 Migration — removed 'using System.Web.SessionState;' (not needed in ASP.NET Core).
// NOTE: .NET 10 Migration — added 'using Microsoft.Extensions.Caching.Memory;' to replace HttpApplicationState
// (System.Web.HttpApplicationState is not available in .NET 10; IMemoryCache is the ASP.NET Core equivalent).
...
namespace Spring.Social.Office365
{
	public class Office365Sync
	{
		...
		// NOTE: .NET 10 Migration — replaced 'HttpApplicationState Application' parameter with 'IMemoryCache memoryCache'.
		// HttpApplicationState (System.Web) is not available in .NET 10; IMemoryCache is the ASP.NET Core equivalent.
		public static Spring.Social.Office365.Api.IOffice365 CreateApi(IMemoryCache memoryCache, string sOAuthAccessToken)
		{
...
namespace SplendidCRM
{
	public class Office365Utils
	{
		...
		// NOTE: .NET 10 Migration — replaced 'HttpApplicationState Application' parameter with 'IMemoryCache memoryCache'.
		// HttpApplicationState (System.Web) is not available in .NET 10; IMemoryCache is the ASP.NET Core equivalent.
		public static void SendTestMessage(IMemoryCache memoryCache, Guid gUSER_ID, string sFromAddress, string sFromName, string sToAddress, string sToName)
		{
			throw(new Exception("Office365 integration is not supported."));
		}
		...
		// NOTE: .NET 10 Migration — replaced 'HttpApplicationState Application' parameter with 'IMemoryCache memoryCache'.
		// HttpApplicationState (System.Web) is not available in .NET 10; IMemoryCache is the ASP.NET Core equivalent.
		public static bool ValidateExchange(IMemoryCache memoryCache, string sOAuthDirectoryTenatID, string sOAuthClientID, string sOAuthClientSecret, Guid gUSER_ID, string sMAILBOX, StringBuilder sbErrors)
		{
			throw(new Exception("Office365 integration is not supported."));
		}
