...
namespace SplendidCRM
{
	...
	public class RulesParser
	{
		...
		static RulesParser()
		{
			...
			if ( typParser != null )
			{
				m_ctorParser               = typParser.GetConstructor(BindingFlags.Instance | BindingFlags.NonPublic, null, new Type[] { typeof(RuleValidation) }, null);
			...
			// NOTE: On .NET 10, System.Workflow.Activities assembly is not available. typParser will be null.
			// This is expected for this dormant stub. The class will compile but methods will throw at runtime.
		}

		public RulesParser(RuleValidation validation)
		{
			if ( m_ctorParser != null )
				objParser = m_ctorParser.Invoke(new object[] { validation } );
		}

		public RuleExpressionCondition ParseCondition(string expressionString)
		{
			if ( m_methParseCondition == null )
				return null;
			try
			{
		...
		{
			if ( m_methParseStatementList == null )
				return null;
			try
			{
		...
		{
			if ( m_methParseSingleStatement == null )
				return null;
			try
			{
