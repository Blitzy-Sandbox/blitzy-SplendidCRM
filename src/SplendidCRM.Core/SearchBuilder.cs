 ...
 *********************************************************************************************************************/
// Migrated from SplendidCRM/_code/SearchBuilder.cs for .NET 10 ASP.NET Core (net10.0).
// No System.Web dependencies existed in this file; no HttpContext.Current usage present.
// Minimal change: preserved all SQL WHERE clause generation logic, provider-aware LIKE escaping,
// boolean/equality/wildcard operator handling, and custom OData-style query parameter parsing
// ($filter, $select, $orderby, $groupby). This is NOT standard OData — custom parsing is preserved exactly.
using System;
using System.Data;
using System.Text;
...
{
	/// <summary>
	/// Provider-aware SQL WHERE clause generation for full-text search-style queries.
	/// Supports custom OData-style query parameter parsing ($filter, $select, $orderby, $groupby).
	/// NOTE: This is NOT standard OData middleware — it is custom parsing logic preserved from the
	/// legacy system. The custom parsing in ParseInput() and BuildQuery() must remain byte-identical
	/// to preserve 100% SQL predicate parity with the .NET Framework 4.8 baseline.
	/// </summary>
	public class SearchBuilder
	{
		...
		/// <summary>
		/// Returns the parsed search tokens extracted from the input string during construction.
		/// Tokens are split on whitespace, commas, semicolons, and operator symbols.
		/// </summary>
		...
		/// Escapes backslashes, percent signs, and underscores to prevent them from
		/// acting as LIKE wildcard characters. Provider-aware escape handling is applied
		/// by the calling BuildQuery() method.
		/// </summary>
		...
		/// <summary>
		/// Tokenizes the input search string into individual search terms.
		/// Supports quoted strings, operator symbols (+, -, &lt;, &gt;, !, =),
		/// and whitespace/comma/semicolon delimiters.
		/// For Oracle, DB2, and PostgreSQL the input is upper-cased for case-insensitive matching
		/// (SQL Server uses collation-based case-insensitivity so upper-casing is not applied there).
		/// Uses ControlChars.Cr, ControlChars.Lf, and ControlChars.Tab as whitespace token separators.
		/// </summary>
		...
		/// <summary>
		/// Builds a SQL WHERE clause fragment for the given field from the current token set.
		/// Assumes the field is a text field. Equivalent to calling BuildQuery(sCondition, sField, bTextField: true).
		/// </summary>
		...
		/// <summary>
		/// Parses sInput then builds a SQL WHERE clause fragment for the given field.
		/// </summary>
		...
		/// <summary>
		/// Determines whether the current token set produces a non-empty WHERE clause.
		/// Used to validate user queries before building the full SQL predicate.
		/// The goal is to avoid queries like "" AND "" where nothing is provided in the quotes
		/// — BuildQuery will properly return nothing, but IsValidQuery() lets callers check this up-front.
		/// </summary>
		...
		/// <summary>
		/// Builds a SQL WHERE clause fragment for the given field using the current token set.
		/// Assumes the field is a text field (bTextField: true).
		/// Uses Strings.Space() for alignment padding in the output SQL.
		/// </summary>
		...
		/// <summary>
		/// Core WHERE clause builder. Iterates over the parsed token list and generates
		/// a SQL fragment with LIKE/= conditions, NOT modifiers, OR/AND conjunctions,
		/// and comparison operators (&lt;, &lt;=, &gt;, &gt;=). Provider-specific LIKE ESCAPE
		/// clauses are emitted for MySQL and PostgreSQL. SQL Server uses N'...' national
		/// character literals for Unicode support (e.g., Farsi text).
		/// </summary>
