...
namespace SplendidCRM
{
	...
	/// Provider-agnostic database factory using reflection to dynamically load and instantiate
	/// database provider types (connections, commands, adapters, parameters).
	/// Migrated from .NET Framework 4.8 to .NET 10: Assembly.LoadWithPartialName replaced with Assembly.Load.
	/// </summary>
	public class DbProviderFactory
	{
		...
		public DbProviderFactory(string sConnectionString, string sAssemblyName, string sConnectionName, string sCommandName, string sDataAdapterName, string sParameterName, string sBuilderName)
		{
			...
			// .NET 10 Migration: Assembly.LoadWithPartialName is obsolete and removed.
			// Use Assembly.Load which resolves assemblies by name from the application's dependency context.
			// For NuGet-referenced assemblies (e.g., Microsoft.Data.SqlClient), the assembly is available
			// in the runtime's load context when the project has a PackageReference to it.
			m_asmSqlClient      = Assembly.Load(sAssemblyName);
