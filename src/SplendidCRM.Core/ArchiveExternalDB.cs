 ...
 *********************************************************************************************************************/
// .NET 10 Migration: SplendidCRM/_code/ArchiveExternalDB.cs → src/SplendidCRM.Core/ArchiveExternalDB.cs
// Changes applied:
//   - REMOVED: using System.Web; (HttpApplicationState is a System.Web type removed in ASP.NET Core)
//   - ADDED:   using Microsoft.AspNetCore.Http; (HttpContext parameter type preserved in all method signatures)
//   - ADDED:   using Microsoft.Extensions.Caching.Memory; (IMemoryCache replaces Application[])
//   - REPLACED: static class → DI-injectable instance class with constructor injection
//   - ADDED:   DI constructor accepting IMemoryCache, DbProviderFactories, DbAcrhiveFactories
//   - REPLACED: private static methods → private instance methods (removed static keyword)
//   - REPLACED: DbProviderFactories.GetFactory(Context.Application) → _dbProviderFactories.GetFactory(_memoryCache)
//   - REPLACED: DbAcrhiveFactories.GetFactory(Context.Application)  → _dbArchiveFactories.GetFactory(_memoryCache)
//   - REPLACED: Context.Application["CONFIG.Archive.BatchSize"]     → _memoryCache.Get<object>("CONFIG.Archive.BatchSize")
//   - REPLACED: Context.Application["ArchiveConnectionString"]      → _memoryCache.Get<object>("ArchiveConnectionString")
//   - PRESERVED: namespace SplendidCRM, all public signatures (bInsideArchive, dtLastBuild, RunArchive(Object))
//   - PRESERVED: public static bool bInsideArchive and public static DateTime dtLastBuild (shared reentrancy state)
//   - PRESERVED: HttpContext Context parameter throughout private methods (passed to SplendidError.SystemMessage)
//   - PRESERVED: All business logic, SQL queries, data movement patterns, and error handling
//   - PRESERVED: #if DEBUG / #if false conditional compilation blocks
...
namespace SplendidCRM
{
	...
	/// Migrated from SplendidCRM/_code/ArchiveExternalDB.cs for .NET 10 ASP.NET Core.
	/// 
	/// Manages external archive database operations: building archive schema, moving data from the
	/// main CRM database to an external archive database based on archive rules, and maintaining
	/// archive table structure (columns, indexes) in sync with the main database.
	/// 
	/// BEFORE (.NET Framework 4.8):
	///   Static class; all methods static; HttpApplicationState passed as parameter.
	///   Application["key"] reads for database factory and configuration values.
	///   Usage: ArchiveExternalDB.RunArchive(httpContext);
	/// 
	/// AFTER (.NET 10 ASP.NET Core):
	///   DI-injectable instance class. IMemoryCache replaces Application[].
	///   DbProviderFactories / DbAcrhiveFactories injected for database connections.
	///   Usage: _archiveExternalDB.RunArchive(httpContext); (from ArchiveHostedService)
	/// </summary>
	public class ArchiveExternalDB
	{
		// =====================================================================================
		// Shared static reentrancy guard — preserved from source (not converted to instance fields)
		// so that external callers (ArchiveHostedService) can inspect them without holding an
		// instance reference. The same guard semantics are preserved: only one archive operation
		// runs at a time across the entire process.
		// =====================================================================================

		/// <summary>
		/// Reentrancy guard. Set to true when a RunArchive invocation is in progress.
		/// Prevents concurrent archive runs from the IHostedService timer.
		/// BEFORE: public static bool bInsideArchive = false;
		/// AFTER:  unchanged — preserved as static for backward compatibility with ArchiveHostedService.
		/// </summary>
		public static bool     bInsideArchive = false;

		/// <summary>
		/// Tracks the last time the archive schema build (BuildArchiveTable / SqlBuildArchiveIndexes) ran.
		/// The build is throttled to execute at most once every 6 hours.
		/// BEFORE: public static DateTime dtLastBuild = DateTime.MinValue;
		/// AFTER:  unchanged — preserved as static for backward compatibility.
		/// </summary>
		public static DateTime dtLastBuild    = DateTime.MinValue;

		// =====================================================================================
		// DI-injected fields
		// BEFORE: All methods received Context.Application (HttpApplicationState) as a parameter.
		// AFTER:  IMemoryCache replaces Application[]; DbProviderFactories / DbAcrhiveFactories
		//         are injected for database connection creation.
		// =====================================================================================

		/// <summary>
		/// Replaces HttpApplicationState (Application[]) for reading cached database provider
		/// and configuration values (CONFIG.Archive.BatchSize, ArchiveConnectionString).
		/// BEFORE: Context.Application["key"]
		/// AFTER:  _memoryCache.Get&lt;object&gt;("key")
		/// </summary>
		private readonly IMemoryCache        _memoryCache        ;

		/// <summary>
		/// Replaces static call DbProviderFactories.GetFactory(Context.Application).
		/// BEFORE: DbProviderFactory dbf = DbProviderFactories.GetFactory(Context.Application);
		/// AFTER:  DbProviderFactory dbf = _dbProviderFactories.GetFactory(_memoryCache);
		/// </summary>
		private readonly DbProviderFactories _dbProviderFactories;

		/// <summary>
		/// Replaces static call DbAcrhiveFactories.GetFactory(Context.Application).
		/// BEFORE: DbProviderFactory dbfArchive = DbAcrhiveFactories.GetFactory(Context.Application);
		/// AFTER:  DbProviderFactory dbfArchive = _dbArchiveFactories.GetFactory(_memoryCache);
		/// Note: intentional spelling preserved ("Acrhive") for backward compatibility.
		/// </summary>
		private readonly DbAcrhiveFactories  _dbArchiveFactories ;
		...
		/// <summary>
		/// Initializes a new instance of <see cref="ArchiveExternalDB"/> with DI-injected services.
		/// This constructor replaces the static-only access pattern of the .NET Framework 4.8 version.
		/// </summary>
		/// <param name="memoryCache">
		/// IMemoryCache replacing Application[] for reading CONFIG.Archive.BatchSize and ArchiveConnectionString.
		/// Also passed to DbProviderFactories.GetFactory(IMemoryCache) and DbAcrhiveFactories.GetFactory(IMemoryCache).
		/// </param>
		/// <param name="dbProviderFactories">
		/// Main CRM database factory. Replaces the static DbProviderFactories.GetFactory(Context.Application) pattern.
		/// </param>
		/// <param name="dbArchiveFactories">
		/// Archive database factory. Replaces the static DbAcrhiveFactories.GetFactory(Context.Application) pattern.
		/// Note: parameter name uses standard spelling; class name preserves original "Acrhive" typo.
		/// </param>
		public ArchiveExternalDB(
			IMemoryCache         memoryCache         ,
			DbProviderFactories  dbProviderFactories ,
			DbAcrhiveFactories   dbArchiveFactories  )
		{
			_memoryCache         = memoryCache        ;
			_dbProviderFactories = dbProviderFactories;
			_dbArchiveFactories  = dbArchiveFactories ;
		}

		// =====================================================================================
		// Private helper methods
		// BEFORE: private static string TabSpace(int nNumber)
		// AFTER:  private string TabSpace(int nNumber)   — static keyword removed
		// Business logic unchanged; Strings.Space() from VisualBasic.cs still used.
		// =====================================================================================
		...
		// =====================================================================================
		// BuildArchiveTable
		// BEFORE: private static string BuildArchiveTable(HttpContext Context, string sTABLE_NAME)
		//   Calls DbProviderFactories.GetFactory(Context.Application)
		//   Calls DbAcrhiveFactories.GetFactory(Context.Application)
		// AFTER:  private string BuildArchiveTable(HttpContext Context, string sTABLE_NAME)
		//   Calls _dbProviderFactories.GetFactory(_memoryCache)
		//   Calls _dbArchiveFactories.GetFactory(_memoryCache)
		//   HttpContext Context parameter preserved for passing to SplendidError.SystemMessage.
		// =====================================================================================

		private string BuildArchiveTable(HttpContext Context, string sTABLE_NAME)
		{
			...
			// .NET 10 Migration: DbProviderFactories.GetFactory(Context.Application) → _dbProviderFactories.GetFactory(_memoryCache)
			...
			using ( IDbConnection con = dbf.CreateConnection() )
			{
				...
				if ( bEXISTS )
				{
					...
					using ( DataTable dt = new DataTable() )
					{
						...
						// .NET 10 Migration: DbAcrhiveFactories.GetFactory(Context.Application) → _dbArchiveFactories.GetFactory(_memoryCache)
						DbProviderFactory dbfArchive = _dbArchiveFactories.GetFactory(_memoryCache);
		...
		// =====================================================================================
		// SqlBuildArchiveIndexes
		// BEFORE: private static string SqlBuildArchiveIndexes(HttpContext Context, string sTABLE_NAME)
		//   Calls DbProviderFactories.GetFactory(Context.Application)
		//   Calls DbAcrhiveFactories.GetFactory(Context.Application)
		// AFTER:  private string SqlBuildArchiveIndexes(HttpContext Context, string sTABLE_NAME)
		//   Calls _dbProviderFactories.GetFactory(_memoryCache)
		//   Calls _dbArchiveFactories.GetFactory(_memoryCache)
		// =====================================================================================

		private string SqlBuildArchiveIndexes(HttpContext Context, string sTABLE_NAME)
		{
			StringBuilder sb = new StringBuilder();
			// .NET 10 Migration: DbProviderFactories.GetFactory(Context.Application) → _dbProviderFactories.GetFactory(_memoryCache)
			DbProviderFactory dbf = _dbProviderFactories.GetFactory(_memoryCache);
		...
		// =====================================================================================
		// ArchiveBuildAll
		// BEFORE: private static string ArchiveBuildAll(HttpContext Context)
		//   Calls DbProviderFactories.GetFactory(Context.Application)
		// AFTER:  private string ArchiveBuildAll(HttpContext Context)
		//   Calls _dbProviderFactories.GetFactory(_memoryCache)
		// BuildArchiveTable calls are now instance method calls.
		// =====================================================================================

		private string ArchiveBuildAll(HttpContext Context)
		{
			StringBuilder sb = new StringBuilder();
			// .NET 10 Migration: DbProviderFactories.GetFactory(Context.Application) → _dbProviderFactories.GetFactory(_memoryCache)
			DbProviderFactory dbf = _dbProviderFactories.GetFactory(_memoryCache);
		...
		// MIGRATED: private static → private; DbProviderFactories.GetFactory(Context.Application) → _dbProviderFactories.GetFactory(_memoryCache)
		// MIGRATED: DbAcrhiveFactories.GetFactory(Context.Application) → _dbArchiveFactories.GetFactory(_memoryCache)
		private string BuildInsertProcedures(HttpContext Context, string sTABLE_NAME, ref IDbCommand cmdPrimaryInsert, ref IDbCommand cmdCustomInsert)
		{
			sTABLE_NAME = sTABLE_NAME.ToUpper();
			string sCUSTOM_TABLE   = sTABLE_NAME  + "_CSTM";
		...
		// MIGRATED: private static → private; factory calls replaced with injected instances
		private string ArchiveMoveData(HttpContext Context, Guid gID, string sTABLE_NAME, IDbCommand cmdPrimaryInsert, IDbCommand cmdCustomInsert)
		{
		...
		// MIGRATED: private static → private; factory calls replaced with injected instances
		private string ArchiveMoveAuditData(HttpContext Context, Guid gAUDIT_ID, string sTABLE_NAME, IDbCommand cmdPrimaryInsert, IDbCommand cmdCustomInsert)
		{
			if ( cmdPrimaryInsert == null )
				return String.Empty;
		...
		// MIGRATED: private static → private; factory calls replaced with injected instances
		private string ArchiveMoveRelationshipData(HttpContext Context, Guid gID, string sMODULE_NAME, string sTABLE_NAME)
		{
			StringBuilder sb = new StringBuilder();
			DbProviderFactory dbf = _dbProviderFactories.GetFactory(_memoryCache);
			using ( IDbConnection con = dbf.CreateConnection() )
		...
		// MIGRATED: private static → private; factory calls replaced with injected instances
		private string ArchiveMoveActivitiesData(HttpContext Context, Guid gID, string sMODULE_NAME, string sTABLE_NAME)
		{
		...
		// MIGRATED: private static → private; factory calls replaced with injected instances
		private string ArchiveMoveRelatedData(HttpContext Context, Guid gID, string sMODULE_NAME, string sTABLE_NAME)
		{
			StringBuilder sb = new StringBuilder();
			DbProviderFactory dbf = _dbProviderFactories.GetFactory(_memoryCache);
			using ( IDbConnection con = dbf.CreateConnection() )
		...
		// MIGRATED: private static → private; factory calls replaced with injected instances
		// MIGRATED: Context.Application["CONFIG.Archive.BatchSize"] → _memoryCache.Get<object>("CONFIG.Archive.BatchSize")
		private string ArchiveRulesRunAll(HttpContext Context)
		{
			StringBuilder sb = new StringBuilder();
			DbProviderFactory dbf = _dbProviderFactories.GetFactory(_memoryCache);
			using ( IDbConnection con = dbf.CreateConnection() )
					...
					using ( DbDataAdapter da = dbf.CreateDataAdapter() )
					{
						...
						using ( DataTable dtRules = new DataTable() )
						{
							...
							foreach ( DataRow rowRule in dtRules.Rows )
							{
								...
								// MIGRATED: Context.Application["CONFIG.Archive.BatchSize"] → _memoryCache.Get<object>("CONFIG.Archive.BatchSize")
								int nMaxArchiveBatchSize = Sql.ToInteger(_memoryCache.Get<object>("CONFIG.Archive.BatchSize"));
		...
		// MIGRATED: public static void → public void (instance method); factory calls replaced with injected instances
		// MIGRATED: Context.Application["ArchiveConnectionString"] → _memoryCache.Get<object>("ArchiveConnectionString")
		public void RunArchive(Object sender)
		{
			...
			if ( !bInsideArchive )
			{
				...
				try
				{
					...
					// 07/10/2018 Paul.  Before we run an archive, lets first test if we can connect to the archive database. 
					try
					{
						DbProviderFactory dbfArchive = _dbArchiveFactories.GetFactory(_memoryCache);
						using ( IDbConnection conArchive = dbfArchive.CreateConnection() )
