# =============================================================================
# SplendidCRM ECS Fargate — Prod Environment Root Module
# =============================================================================
#
# Purpose:
#   Root module instantiation file for the PRODUCTION environment. This file
#   calls the shared common module (../../modules/common) and passes all 22
#   required variables sourced from variables.tf, data.tf, and locals.tf.
#
# Architecture:
...
#     ├── locals.tf          — Prod-specific sizing constants (ECS, RDS, observability)
#     ├── main.tf            — THIS FILE: module instantiation + output passthrough
#     └── prod.auto.tfvars   — Prod-specific variable values
#
...
module "common" {
  ...
  # ---------------------------------------------------------------------------
  # Identity & Configuration Variables (from variables.tf → prod.auto.tfvars)
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # ECS Fargate Task Sizing (from locals.tf — Production Tier)
  # ---------------------------------------------------------------------------
  # Production sizing: 2048 CPU, 4096 MB memory, 50 GB ephemeral, 2-10 tasks
  # These values are uniform for both backend and frontend tasks per ACME
  # standard. Production uses the maximum tier for sustained workloads and
  # high-availability requirements with auto-scaling up to 10 tasks.
  # ---------------------------------------------------------------------------
  ...
  #   - VPC: tag-based discovery per ACME naming convention
  ...
  # ---------------------------------------------------------------------------
  # RDS SQL Server Sizing (from locals.tf — Production Tier)
  # ---------------------------------------------------------------------------
  # Production-grade memory-optimized instance sizing:
  #   - db.r5.large: 2 vCPU, 16 GB RAM — memory-optimized for sustained
  #     SQL Server workloads under production load
  #   - 100 GB initial storage with autoscaling up to 500 GB
  #   - Multi-AZ, 35-day backup retention, and deletion protection enabled
  #     via common module conditional (var.environment == "prod")
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # ACME Tag Variables (from variables.tf → prod.auto.tfvars)
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # 90-day CloudWatch log retention for production — meets compliance
  # requirements for production audit trails and operational forensics.
  # ---------------------------------------------------------------------------
  log_retention_days = local.log_retention_days

  # ---------------------------------------------------------------------------
  # TLS/Certificate Configuration (from variables.tf → prod.auto.tfvars)
  # ---------------------------------------------------------------------------
  ...
  # ALB. Production environments SHOULD always provide a certificate for
