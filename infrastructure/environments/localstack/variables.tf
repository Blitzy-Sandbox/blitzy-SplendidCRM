# -----------------------------------------------------------------------------
# SplendidCRM ECS Fargate — LocalStack Environment Variables
# -----------------------------------------------------------------------------
# Defines ALL Terraform input variables for the localstack validation
# environment root module. These variables are populated by
# localstack.auto.tfvars and consumed by main.tf when passing values to the
# common module.
#
...
variable "name_prefix" {
  type        = string
  description = "Naming prefix for all resources (e.g., splendidcrm-local). Used in resource names, tags, and log group paths following ACME naming convention: {app}-{env}."
}
