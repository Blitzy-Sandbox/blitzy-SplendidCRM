# =============================================================================
# SplendidCRM ECS Fargate — Staging Environment Root Module
# =============================================================================
#
# Purpose:
#   Root module instantiation file for the STAGING environment. This file calls
...
#     ├── locals.tf            — Staging-specific sizing constants (ECS, RDS, observability)
#     ├── main.tf              — THIS FILE: module instantiation + output passthrough
#     └── staging.auto.tfvars  — Staging-specific variable values
#
...
module "common" {
  ...
  # ---------------------------------------------------------------------------
  # Identity & Configuration Variables (from variables.tf → staging.auto.tfvars)
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # ECS Fargate Task Sizing (from locals.tf — Staging Tier)
  # ---------------------------------------------------------------------------
  # Staging sizing: 1024 CPU, 2048 MB memory, 30 GB ephemeral, 2-6 tasks
  # These values are uniform for both backend and frontend tasks per ACME
  # standard. Staging provides mid-range resources between dev (512/1024) and
  # production (2048/4096) for pre-release validation workloads.
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # RDS SQL Server Sizing (from locals.tf — Staging Tier)
  # ---------------------------------------------------------------------------
  # Mid-range instance sizing for staging workloads:
  #   - db.t3.large: 2 vCPU, 8 GB RAM — suitable for staging SQL Server
  #   - 50 GB initial storage with autoscaling up to 200 GB
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # ACME Tag Variables (from variables.tf → staging.auto.tfvars)
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # 60-day CloudWatch log retention for staging — longer than dev (30) but
  # shorter than production (90) to balance observability with storage costs.
  # ---------------------------------------------------------------------------
  log_retention_days = local.log_retention_days

  # ---------------------------------------------------------------------------
  # TLS/Certificate Configuration (from variables.tf → staging.auto.tfvars)
  # ---------------------------------------------------------------------------
