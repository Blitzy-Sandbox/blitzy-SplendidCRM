# -----------------------------------------------------------------------------
# SplendidCRM ECS Fargate — Staging Environment Variables
# -----------------------------------------------------------------------------
# Defines ALL Terraform input variables for the staging environment root module.
# These variables are populated by staging.auto.tfvars and consumed by main.tf
...
variable "name_prefix" {
  type        = string
  description = "Naming prefix for all resources (e.g., splendidcrm-staging). Used in resource names, tags, and log group paths following ACME naming convention: {app}-{env}."
}
